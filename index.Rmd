...
impact <- d %>% 
  ...
  mutate(headache_migraine_cluster_score = pcss_headache + pcss_light_sensitivity + pcss_noise_sensitivity + pcss_numbness,
         cognitive_cluster_score = pcss_difficulty_concentrating + pcss_difficulty_remembering + pcss_feeling_slow + pcss_feeling_foggy,
         anxiety_mood_cluster_score = pcss_sadness + pcss_irritability + pcss_more_emotional + pcss_nervousness,
...
impact_export <- impact %>% 
  select(-c(3:4))

write.csv(impact_export, "impact_data.csv")

baseline_data <- baseline_data %>% 
  select(passport_id, test_date, test_type)

# widen baseline tests 

baseline_wide <- baseline_data %>% 
  group_by(passport_id, test_type) %>% 
  mutate(row = row_number()) %>% 
  arrange(passport_id) %>% 
  select(row, passport_id, test_date, test_type)




baseline_wide$test_date <- as.Date(baseline_wide$test_date, format = "%m/%d/%Y")

baseline_dates <- baseline_wide %>% 
  rename(test_date_baseline = test_date,
         test_type_baseline = test_type)

write.csv(baseline_dates, "baseline_dates.csv")
...
baseline_data$test_date <- as.Date(baseline_data$test_date, format = "%m/%d/%Y")
...
create_react_time(all_four_pi_pcss, dys_btwn_tst_1_3)
...
```{r, include=FALSE}
# attempting to join baseline data set to post-injury sets to determine duration of time from baseline test to post-injury test

one_test_baseline <- right_join(baseline_data, all_one_pi_pcss)


one_test_baseline <- one_test_baseline %>% 
  mutate(dys_btwn_pi_bl = test_date_post_injury_1 - test_date) %>% 
  mutate(dys_btwn_pi_bl = as.numeric(dys_btwn_pi_bl)) %>% 
  filter(dys_btwn_pi_bl >= 0)

summary(one_test_baseline$dys_btwn_pi_bl)
...
all_leagues <- all_leagues %>% 
  ...
  mutate(dys_btwn_onset_rtp_7 = rtp_protocol_7 - onset,
         dys_btwn_onset_rtp_3 = rtp_protocol_3_light_exercise - onset,
         dys_btwn_rtp_3_rtp_7 = rtp_protocol_7 - rtp_protocol_3_light_exercise) %>% 
  mutate(dys_btwn_onset_rtp_7 = as.numeric(dys_btwn_onset_rtp_7),
         dys_btwn_onset_rtp_3 = as.numeric(dys_btwn_onset_rtp_3),
         dys_btwn_rtp_3_rtp_7 = as.numeric(dys_btwn_rtp_3_rtp_7))

mean_2(all_leagues$dys_btwn_onset_rtp_7)
mean_2(all_leagues$dys_btwn_onset_rtp_3)
mean_2(all_leagues$dys_btwn_rtp_3_rtp_7)

# filter age range for date ranges 

all_leagues <- all_leagues %>% 
  filter(dys_btwn_onset_rtp_7 >= 0 & dys_btwn_onset_rtp_7 <= 100,
         dys_btwn_onset_rtp_3 >= 0 & dys_btwn_onset_rtp_3 <= 30,
         dys_btwn_rtp_3_rtp_7 >= 0 & dys_btwn_rtp_3_rtp_7 <= 70)
...
# days between onset and step 3

sims_onset_rtp_3 <- sims_wide(all_leagues, student_id, onset, 
                              dys_btwn_onset_rtp_3)

# days between rtp 2 and 7 

sims_rtp_3_7 <- sims_wide(all_leagues, student_id, onset, 
                              dys_btwn_rtp_3_rtp_7)
...
sims_j20 <- left_join(sims_j19, sims_onset_rtp_3)
all_clean_sims <- left_join(sims_j20, sims_rtp_3_7)
...
| :----------- | :------------------------: |
| One post-injury test (not merged with SIMS) | 9,991 |
| One post-injury test (merged with SIMS) | 254 |
| Two post-injury tests (not merged with SIMS) | 5,295 |
| Two post-injury tests (merged with SIMS) | 336 |
| Three post-injury tests (not merged with SIMS) | 2,228 |
| Three post-injury tests (merged with SIMS) | 133 |
| Four post-injury tests (not merged with SIMS) | 780 |
| Four post-injury tests (merged with SIMS) | 32 |
...
### Non-merged Data Summary 

Overall, average symptom severity decreases with consecutive testing. Also, the average symptom severity score is lower for individuals who complete one post-injury test that individuals who complete multiple post-injury tests, suggesting symptom severity on the first post-injury test is predictive of requiring multiple post-injury tests.

**One post-injury test**

9,991 concussion injuries 

| **Symptom Cluster** | **Mean Test 1** | **SD Test 1** |
| :------------------ | :-------------: | :-----------: |
| Headache-Migraine Cluster | 2.45      | 3.2           | 
| Anxiety-Mood Cluster      | 1.57      | 3.39          |
| Cognitive Cluster         | 2.36      | 3.9           |
| Ocular-Motor Cluster      | 0.29      | 0.82          |
| Vestibular Cluster        | 1.57      | 2.82          |
| Sleep Cluster             | 2.45      | 3.85          |
| Total Symptom Score       | 10.7      | 15.57         |


**Two post-injury tests**

5,295 concussion injuries 

| **Symptom Cluster** | **Mean Test 1** | **SD Test 1** | **Mean Test 2** | **SD Test 2** |
| :-------------------| :-------------: | :-----------: | :-------------: | :-----------: |
| Headache-Migraine Cluster | 3.02      | 3.4           | 0.94            | 1.89          |
| Anxiety-Mood Cluster      | 1.91      | 3.68          | 0.56            | 1.88          |
| Cognitive Cluster         | 2.98      | 4.23          | 0.95            | 2.37          |
| Ocular-Motor Cluster      | 0.36      | 0.92          | 0.1             | 0.44          |
| Vestibular Cluster        | 2.01      | 3.1           | 0.45            | 1.37          |
| Sleep Cluster             | 3.02      | 4.16          | 0.94            | 2.23          |
| Total Symptom Score       | 13.31     | 16.76         | 3.94            | 8.62          |


**Three post-injury tests**

2,228 concussion injuries 

| **Symptom Cluster** | **Mean Test 1** | **SD Test 1** | **Mean Test 2** | **SD Test 2** | **Mean Test 3** | **SD Test 3** | 
| :-------------------| :-------------: | :-----------: | :-------------: | :-----------: | :-----------: | :-----------: | 
| Headache-Migraine Cluster | 3.55   | 3.6        | 1.34         | 2.23       | 0.57 | 1.45 |
| Anxiety-Mood Cluster      | 2.32   | 3.99       | 0.76         | 2.17       | 0.39 | 1.62 |
| Cognitive Cluster         | 3.68   | 4.67       | 1.39         | 2.86       | 0.63 | 1.98 |
| Ocular-Motor Cluster      | 0.45   | 1          | 0.13         | 0.5        | 0.06 | 0.33 |
| Vestibular Cluster        | 2.48   | 3.39       | 0.68         | 1.66       | 0.26 | 1.06 |
| Sleep Cluster             | 3.62   | 4.47       | 1.37         | 2.69       | 0.59 | 1.79 |
| Total Symptom Score       | 16.09  | 18.1       | 5.68         | 10.28      | 2.49 | 6.96 | 


**Four post-injury tests**

780 concussion injuries 

| **Symptom Cluster** | **Mean Test 1** | **SD Test 1** | **Mean Test 2** | **SD Test 2** | **Mean Test 3** | **SD Test 3** | **Mean Test 4** | **SD Test 4** |
| :-------------------| :-------------: | :-----------: | :-------------: | :-----------: | :-----------: | :-----------: | :-----------: | :-----------: |
| Headache-Migraine Cluster | 4.04  | 3.66  | 1.83 | 2.54  | 0.99 | 1.91 | 0.6  | 1.66 |
| Anxiety-Mood Cluster      | 2.7   | 4.3   | 1.07 | 2.64  | 0.63 | 2.04 | 0.43 | 1.99 |
| Cognitive Cluster         | 4.31  | 5.02  | 2.08 | 3.5   | 1.12 | 2.65 | 0.68 | 2.37 |
| Ocular-Motor Cluster      | 0.56  | 1.15  | 0.18 | 0.58  | 0.09 | 0.41 | 0.07 | 0.41 |
| Vestibular Cluster        | 3.06  | 3.72  | 1.03 | 2.11  | 0.48 | 1.52 | 0.29 | 1.36 |
| Sleep Cluster             | 4.28  | 4.85  | 1.98 | 3.25  | 0.98 | 2.35 | 0.54 | 1.87 |
| Total Symptom Score       | 18.95 | 19.51 | 8.17 | 12.36 | 4.28 | 9.27 | 2.62 | 8.6  |
...
```{r, include=TRUE}
create_react_time(one_pi_pcss_sims, dys_btwn_onset_rtp_3)
```
...
```{r, include=FALSE}
ggplot(one_pi_pcss_sims, aes(dys_btwn_onset_test_1, dys_btwn_onset_rtp_3)) +
  geom_point() +
  geom_smooth() +
...
time_scatter(one_pi_pcss_sims, dys_btwn_onset_test_1, dys_btwn_onset_rtp_3) +
  labs(x = "Onset to Test 1 Date",
       y = "Onset to RTL Completion")
```
...
```{r, include=TRUE}
time_scatter(one_pi_pcss_sims, dys_btwn_onset_rtp_3, dys_btwn_onset_rtp_7) +
...
```{r, include=TRUE}
create_react_time(two_pi_pcss_sims, dys_btwn_onset_rtp_3)
```
...
```{r, include=TRUE}
time_scatter(two_pi_pcss_sims, dys_btwn_onset_test_1, dys_btwn_onset_rtp_3) +
  labs(x = "Onset to Test 1 Date",
       y = "Onset to RTL Completion")
```
...
##### Plot 3
```{r, include=TRUE}
time_scatter(two_pi_pcss_sims, dys_btwn_onset_test_2, dys_btwn_onset_rtp_3) +
...
##### Plot 6
```{r, include=TRUE}
time_scatter(two_pi_pcss_sims, dys_btwn_onset_rtp_3, dys_btwn_onset_rtp_7) +
  labs(x = "Onset to RTL Completion",
       y = "Onset to RTP Completion")
```
...
```{r, include=TRUE}
create_react_time(three_pi_pcss_sims, dys_btwn_onset_rtp_3)
```
...
```{r, include=TRUE}
time_scatter(three_pi_pcss_sims, dys_btwn_onset_test_1, dys_btwn_onset_rtp_3) +
  labs(x = "Onset to Test 1 Date",
       y = "Onset to RTL Completion")
```
...
##### Plot 5
```{r, include=TRUE}
time_scatter(three_pi_pcss_sims, dys_btwn_onset_test_2, dys_btwn_onset_rtp_3) +
  labs(x = "Onset to Test 2 Date",
       y = "Onset to RTL Completion")
```
...
##### Plot 6
```{r, include=TRUE}
time_scatter(three_pi_pcss_sims, dys_btwn_onset_test_3, dys_btwn_onset_rtp_3) +
...
##### Plot 10
```{r, include=TRUE}
time_scatter(three_pi_pcss_sims, dys_btwn_onset_rtp_3, dys_btwn_onset_rtp_7) +
  labs(x = "Onset to RTL Completion",
       y = "Onset to RTP Completion")
```
...
```{r, include=TRUE}
create_react_time(four_pi_pcss_sims, dys_btwn_onset_rtp_3)
```
...
```{r, include=TRUE}
time_scatter(four_pi_pcss_sims, dys_btwn_onset_test_1, dys_btwn_onset_rtp_3) +
  labs(x = "Onset to Test 1 Date",
       y = "Onset to RTL Completion")
```
...
##### Plot 8
```{r, include=TRUE}
time_scatter(four_pi_pcss_sims, dys_btwn_onset_test_2, dys_btwn_onset_rtp_3) +
  labs(x = "Onset to Test 2 Date",
       y = "Onset to RTL Completion")
```
...
##### Plot 9
```{r, include=TRUE}
time_scatter(four_pi_pcss_sims, dys_btwn_onset_test_3, dys_btwn_onset_rtp_3) +
  labs(x = "Onset to Test 3 Date",
       y = "Onset to RTL Completion")
```
...
##### Plot 10
```{r, include=TRUE}
time_scatter(four_pi_pcss_sims, dys_btwn_onset_test_4, dys_btwn_onset_rtp_3) +
...
##### Plot 15
```{r, include=TRUE}
time_scatter(four_pi_pcss_sims, dys_btwn_onset_rtp_3, dys_btwn_onset_rtp_7) +
  labs(x = "Onset to RTL Completion",
       y = "Onset to RTP Completion")
```
...
254 concussion injuries 

| **Benchmark** | **Mean** | **SD** | 
| :------------ | :------: | :----: |
| Onset to Test 1 | 4.96 days | 4.65 days |
| Onset to RTL completion | 11.26 days | 6.05 days |
| Onset to RTP completion | 18.78 days | 8.25 days |

**Two post-injury tests**

336 concussion injuries 

| **Benchmark** | **Mean** | **SD** | 
| :------------ | :------: | :----: |
| Onset to Test 1 | 4.71 days | 4.57 days |
| Onset to Test 2 | 9.29 days | 5.39 days |
| Onset to RTL completion | 12.24 days | 6.22 days |
| Onset to RTP completion | 19.87 days | 8.12 days |

**Three post-injury tests**

133 concussion injuries 

| **Benchmark** | **Mean** | **SD** | 
| :------------ | :------: | :----: |
| Onset to Test 1 | 4.62 days | 4.43 days |
| Onset to Test 2 | 8.88 days | 4.84 days |
| Onset to Test 3 | 13.54 days | 5.88 days |
| Onset to RTL completion | 14.26 days | 6.66 days |
| Onset to RTP completion | 22.89 days | 8.57 days |


**Four post-injury tests**

32 concussion injuries 


| **Benchmark** | **Mean** | **SD** | 
| :------------ | :------: | :----: |
| Onset to Test 1 | 4.47 days   | 4.63 days  |
| Onset to Test 2 | 9.97 days | 5.08 days |
| Onset to Test 3 | 14.69 days | 5.95 days |
| Onset to Test 4 | 21.31 days | 8.29 days |
| Onset to RTL completion | 18.66 days | 6.55 days |
| Onset to RTP completion | 30.03 days | 8.69 days |


Similar to the non-merged data, average total symptom severity scores decrease with consecutive testing. Additionally, the average symptom severity scores on test 1 are higher for individuals who complete multiple post-injury tests. 
...
| **Symptom Cluster** | **Mean Test 1** | **SD Test 1** |
| :------------------ | :-------------: | :-----------: |
| Headache-Migraine Cluster | 2.5       | 3.26         | 
| Anxiety-Mood Cluster      | 1.61      | 3.14          |
| Cognitive Cluster         | 2.33      | 4.0           |
| Ocular-Motor Cluster      | 0.32      | 0.92          |
| Vestibular Cluster        | 1.64      | 2.83          |
| Sleep Cluster             | 2.43      | 3.74          |
| Total Symptom Score       | 10.83      | 15.74         |
...
| **Symptom Cluster** | **Mean Test 1** | **SD Test 1** | **Mean Test 2** | **SD Test 2** |
| :-------------------| :-------------: | :-----------: | :-------------: | :-----------: |
| Headache-Migraine Cluster | 2.76      | 3.25           | 0.65            | 1.51          |
| Anxiety-Mood Cluster      | 1.72      | 3.3          | 0.34            | 1.26          |
| Cognitive Cluster         | 2.79      | 4.14          | 0.68            | 1.88          |
| Ocular-Motor Cluster      | 0.32      | 0.9          | 0.06             | 0.31          |
| Vestibular Cluster        | 1.83      | 2.98           | 0.29            | 0.99          |
| Sleep Cluster             | 2.51      | 3.69          | 0.62            | 1.8          |
| Total Symptom Score       | 11.94     | 16.01         | 2.64            | 6.4          |
...
| **Symptom Cluster** | **Mean Test 1** | **SD Test 1** | **Mean Test 2** | **SD Test 2** | **Mean Test 3** | **SD Test 3** | 
| :-------------------| :-------------: | :-----------: | :-------------: | :-----------: | :-----------: | :-----------: | 
| Headache-Migraine Cluster | 3.11   | 3.63        | 1.14         | 2.08       | 0.37 | 0.83 |
| Anxiety-Mood Cluster      | 2.05   | 3.52       | 0.47         | 1.45       | 0.17 | 0.76 |
| Cognitive Cluster         | 3.28   | 4.64       | 1.15         | 2.62       | 0.33 | 0.91 |
| Ocular-Motor Cluster      | 0.38   | 1.01          | 0.09         | 0.38        | 0.01 | 0.09 |
| Vestibular Cluster        | 2.23   | 3.49       | 0.47         | 1.26       | 0.08 | 0.35 |
| Sleep Cluster             | 2.86   | 4.14       | 1.05         | 2.5       | 0.25 | 0.64 |
| Total Symptom Score       | 13.92  | 18.03       | 4.35         | 8.66      | 0.19 | 0.59 | 
...
| **Symptom Cluster** | **Mean Test 1** | **SD Test 1** | **Mean Test 2** | **SD Test 2** | **Mean Test 3** | **SD Test 3** | **Mean Test 4** | **SD Test 4** |
| :-------------------| :-------------: | :-----------: | :-------------: | :-----------: | :-----------: | :-----------: | :-----------: | :-----------: |
| Headache-Migraine Cluster | 4.25  | 4.13  | 1.41 | 2.46  | 0.78 | 1.24 | 0.41 | 1.46 |
| Anxiety-Mood Cluster      | 2.5   | 3.69  | 0.47 | 1.32  | 0.28 | 0.73 | 0.03 | 0.18 |
| Cognitive Cluster         | 4.06  | 5.63  | 1.25 | 2.68  | 0.66 | 1.36 | 0.41 | 2.12 |
| Ocular-Motor Cluster      | 0.72  | 1.57  | 0.03 | 0.18  | 0.03 | 0.18 | 0    | 0    |
| Vestibular Cluster        | 3.38  | 4.62  | 0.72 | 1.73  | 0.19 | 0.59 | 0.12 | 0.71 |
| Sleep Cluster             | 4.0   | 5.44  | 1.72 | 3.9   | 0.28 | 0.81 | 0.09 | 0.53 |
| Total Symptom Score       | 18.91 | 22.3  | 5.59 | 10.87 | 2.22 | 3.67 | 1.06 | 4.77 |


```{r, include=FALSE}

#exporting data for data science project 

head(all_clean_sims)

sims_concussion_data <- all_clean_sims %>% 
  select(-c(6, 7))

write.csv(sims_concussion_data, "sims_concussion_data.csv")

head(all_one_pi_pcss)

one_post_injury_impact_test <- all_one_pi_pcss %>% 
  select(-c(4, 5))

write.csv(one_post_injury_impact_test, "one_post_injury_impact_test.csv")

head(all_two_pi_pcss)

two_post_injury_impact_test <- all_two_pi_pcss %>% 
  select(-c(5, 6))

write.csv(two_post_injury_impact_test, "two_post_injury_impact_test.csv")

head(all_three_pi_pcss)

three_post_injury_impact_test <- all_three_pi_pcss %>% 
  select(-c(6, 7))

write.csv(three_post_injury_impact_test, "three_post_injury_impact_test.csv")

head(all_four_pi_pcss)

four_post_injury_impact_test <- all_four_pi_pcss %>% 
  select(-c(7, 8))

write.csv(four_post_injury_impact_test, "four_post_injury_impact_test.csv")

head(one_pi_pcss_sims)

one_post_injury_test_impact_sims_merge <- one_pi_pcss_sims %>% 
  select(-c(8, 9))

write.csv(one_post_injury_test_impact_sims_merge, "one_post_injury_test_impact_sims_merge.csv")



two_post_injury_test_impact_sims_merge <- two_pi_pcss_sims %>% 
  select(-c(9, 10))

write.csv(two_post_injury_test_impact_sims_merge, "two_post_injury_test_impact_sims_merge.csv")



three_post_injury_test_impact_sims_merge <- three_pi_pcss_sims %>% 
  select(-c(10, 11))

write.csv(three_post_injury_test_impact_sims_merge, "three_post_injury_test_impact_sims_merge.csv")



four_post_injury_test_impact_sims_merge <- four_pi_pcss_sims %>% 
  select(-c(11, 12))

write.csv(four_post_injury_test_impact_sims_merge, "four_post_injury_test_impact_sims_merge.csv")
